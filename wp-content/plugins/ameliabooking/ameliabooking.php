...
class Plugin
{
        ...
        if ($settingsService->getSetting('activation', 'deleteTables')) {
            ...
            foreach (['/amelia/css', '/amelia/files/tmp', '/amelia/files', '/amelia'] as $path) {
                if (is_dir(UPLOADS_PATH . $path)) {
                    self::deleteFolderContent(UPLOADS_PATH . $path);
                    rmdir(UPLOADS_PATH . $path);
                }
            }
        }
    }
    ...
    public static function elementor_popup_notice(){
        global $pagenow;
        if ( $pagenow == 'edit.php' && $_REQUEST['post_type'] === 'elementor_library' && $_REQUEST['tabs_group'] === 'popup') {
            echo "<div class='notice notice-warning'>
             <p>" . BackendStrings::getCommonStrings()['elementor_popup_notice'] . "</p>
         </div>";
        }
    }
}
